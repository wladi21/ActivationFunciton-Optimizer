...
class r2d2_ReplayMemory:
    def __init__(self, capacity, obs_dim, act_dim, args):
        ...
        self.buffer_learn_forward_len = np.zeros(
    ...
    def push(self, ep_states, ep_actions, ep_rewards, ep_hiddens):
        ...
        discounted_sum = [
            [
                sum_rewards(ep_rewards_[x + y: x + y +
                            self.forward_len], self.gamma)
        ...
        for i in range(len(hidden_list)):
            # store burn-in
            self.buffer_burn_in_len[self.position_r2d2] = len(
                burn_in_obs_list[i])
            ...
            self.buffer_learning_len[self.position_r2d2] = len(
                discounted_sum[i])
